...
| Current focus | Contract workflow CLI (`0.3.0`): `nexus-contract init`/`regenerate`/`validate` on top of the shipped `generate`/`push` bin; Strapi deferred to `0.3.1` |
...
- The Strapi provider is deferred from `0.3.0` to `0.3.1` so the contract CLI can be the current milestone; the general-purpose `tooling.cli` remains directional at `0.7.0`.
...
- Strapi and other remote CMS providers beyond WordPress (deferred to `0.3.1`; the former `0.3.0-strapi` milestone was re-scoped to `0.3.0-cli`).
- Content synchronization and change detection.
- Webhooks (WordPress companion outbound webhooks ship in `0.2.7`; generic workflows remain directional `0.5.0`).
