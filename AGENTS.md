...
Strapi is the next directional provider milestone (`0.4.0`), deferred out of `0.3.0` when that milestone was re-scoped to the contract workflow CLI by user decision. It follows the `0.2.1` companion work and the `0.3.0` CLI.
...
The `0.1.6` companion plugin release (root package version stays `0.2.8`, plugin `0.1.6`) fixes two ACF flexible-content wire problems that produced consumer `SchemaError`s. First, the section normalizer now reads the *formatted* ACF value (`get_field( 'nexus_sections', $post->ID )` instead of the unformatted `get_field( ..., false )`), so flexible rows are keyed by their canonical field *names* (`heading`, `body`, ...) rather than their raw ACF field *keys* (`field_nc_flexible_hero_heading`, ...) — previously the prefixed keys leaked into the wire and left every declared component field undefined against the consumer schema. Second, the Features section's `points` are normalized to the canonical flat shape: ACF authors them as a repeater of a single `text` sub-field (`[ { text: "..." } ]`), and the server now flattens them to `points: string[]`, matching `sections.json`, consumer schemas, and Git-authored features; already-flat string lists are left unchanged so Gutenberg/block and Git paths stay equivalent. The plugin artifact is `dist/nexuscontent-0.1.6.zip`.
...
# 33. Required Scope for 0.2.0, 0.2.1, 0.2.2, and 0.2.3
...
- Plugin `0.1.1` source is isolated under `integrations/wordpress/nexuscontent/` and requires WordPress 6.6+ and PHP 8.1+. Version `0.1.1` adds dedicated `posts`, `posts/{id}`, and `posts/slug/{slug}` content routes (the `0.1.0` artifact served posts through the `pages` routes); the package artifact is `dist/nexuscontent-0.1.1.zip`.
...
- Unit tests, PHP lint, PHPCS, PHPStan, contract tests, JavaScript build/lint/format, and packaging pass; `dist/nexuscontent-0.1.1.zip` is the current artifact.
- WordPress integration passes in CI (tests and the escaping gate satisfied). The plugin ships in the release as `dist/nexuscontent-0.1.1.zip`.
...
## 0.2.3 WordPress Companion Consumer (Released)

- Companion collection items surface normalized sections as `data.sections` (`{ type, data }`), the same canonical shape Git blog posts author; `normalizeCompanionPageItem` merges raw fields with the normalized section list, and posts with no sections keep the raw-HTML fallback.
- The built-in `posts` collection routes to the companion `posts` routes via `WordPressCollectionConfig.companionRoute` (`"pages" | "posts"`, default `"posts"`); custom collections without a `companionRoute` fall back to standard REST under `auto` or throw an actionable error under strict `companion`. Companion section media is normalized recursively at the TypeScript boundary (`wire image.url` → `MediaAsset src`), and item `data` surfaces the companion `excerpt` and `featuredImage`.
...
- This CLI is a deliberate, user-approved scope exception to the planned `0.8.0` `tooling.cli`; the general-purpose CLI remains directional and must not be expanded speculatively.
