...
- The three Astro reference consumers (`astro-basic`, `astro-basic-localised`, `astro-wordpress`) move their site-wide layout styles out of `BaseLayout.astro` into `src/styles/global.css` (imported by the shared layout); the WordPress post-prose stylesheet stays separate.

## [0.2.10] - 2026-09-06

Core sections projection and media resolution, SEO service consolidation, and reference-consumer cleanup (root `0.2.10`); the companion plugin fixes `0.2.1`–`0.2.4` and the on-demand preview rework ship alongside.
...
- The `astro-wordpress` reference consumer consolidates its WordPress prose styling into a single bound stylesheet: the prose block formerly shipped inline by `src/app/WordpressContent.astro` (a markup-less stylesheet-only component) moved to `src/styles/wordpress.css`, imported by the blog post and preview pages. `src/app/gutenberg.ts` and `scripts/vendor-gutenberg.mjs` are removed — the two block-style resets (`.wp-block-image`, `.wp-block-table`) that the vendor script downloaded from the `WORDPRESS_API_URL` origin at build time are now scoped rules inside the authored stylesheet, so the example builds with no network step. A consumer whose WordPress fallback content needs WordPress' full block-library CSS can vendor it itself.
- The `astro-basic` and `astro-basic-localised` reference consumers now resolve section media through Core's `nexus.media.resolveFields` like `astro-wordpress` does. The duplicated ~280-line hand-rolled resolvers in each example's `src/app/media.ts` (`resolveHero`, `resolveServices`, `resolveTestimonials`, `resolveGallery`, `resolveBlocks`) are deleted; pages hand whole `page.data` (and blog routes their post body) to `nexus.media.resolveFields`, and the section components tolerate absent media (`image?.src`, optional item lists) with the same optional `{ src, alt }` props the WordPress consumer uses.
...
- The companion plugin `0.2.3` fixes the Gutenberg "Open frontend preview" flow on installs whose `rest_url()` expands into a doubled absolute URL (for example `https://site/wp-json/https://site/wp-json/nexuscontent/v1/preview-token`), which made the token mint fail with `rest_no_route`. The preview script now receives a relative `nexuscontent/v1/` API root and relies on `apiFetch` to resolve it against the site's own REST base, so the mint request is always `<site>/wp-json/nexuscontent/v1/preview-token`. The plugin artifact is `dist/nexuscontent-0.2.3.zip`.
...
- The `astro-wordpress` reference consumer's draft preview now renders companion media and raw-HTML prose with the same formatting as published posts. Previously the preview route read the raw wire sections straight off `envelope.data`, so section media kept the wire `{ url, id, ... }` shape while components read `MediaAsset.src` — every preview image rendered as an empty `<img>`. The route now validates the envelope with `companionPageResponseSchema` and normalizes it with `normalizeCompanionPage` (the exact provider path companion retrieval uses), converting `image.url` to `src` recursively before media resolution; the raw-HTML fallback and the published post page share one prose stylesheet (`src/app/WordpressContent.astro`) instead of the preview's trimmed copy, so drafts, published posts, and the fallback body all format identically (now shared via `src/styles/wordpress.css`). The WordPress-examples CI test asserts the preview HTML now carries `src="…gallery-one.jpg"` through the built server.
- The `astro-wordpress` reference consumer's `preview.astro` route is now on-demand (`export const prerender = false` with the `@astrojs/node` standalone adapter) instead of a static page. Static Astro output prerenders pages without their runtime query string, so the tokenized draft preview permanently rendered the "Missing preview token or content id." notice no matter what params the link carried — under `astro dev` and in a static build alike. The preview route now reads `?token=...&id=...` per request and renders the draft (or an explicit expired/invalid message); every other route stays a static build under `dist/client/`, with `dist/server/entry.mjs` serving the dynamic route. The route also gained an on-failure "Preview diagnostics" block that states whether a token and id reached the renderer and which API base was detected, making a misconfigured `WORDPRESS_API_URL` distinguishable from stripped query strings. The WordPress-examples CI test now boots the built server and exercises both the valid-to-real-draft and invalid-to-expired paths against the local companion API.
...
The CLI is a deliberate, user-approved scope exception to the planned `0.8.0` CLI milestone because the WordPress companion contract is the first capability to genuinely need one; the general-purpose CLI remains planned.
