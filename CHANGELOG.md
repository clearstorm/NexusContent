...
- The three Astro reference consumers now source site identity from the `site` settings content model instead of a hardcoded per-example constant: each `site.json` authors `siteName` (was `siteTitle`) and a new `defaultImage` media field, and each example's consumer-owned `app/seo.ts` exposes `resolvePageSeo(nexus, input, options?)` that fetches those settings and calls Core `resolveSeo` (the localised example passes `{ locale }`). Hardcoded "- NexusContent Example / - NexusContent WordPress Example" title suffixes are gone; page and blog titles now fall back to the authored site name.
