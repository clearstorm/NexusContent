...
| `provider.wordpress.companion-integration` WordPress companion provider integration | in_progress | - | 0.2.1 | Phase 3 provider discovery, contract-version negotiation, companion calls, caching, and fallback are implemented with tests against the repaired contract. A live integration test (`tests/providers/wordpress-companion-live.test.ts`) runs the provider against a fresh Docker wp-env companion install and passes locally; it is wired into CI via the `ts-integration` job. The CI run against the live plugin remains the final gate. |
