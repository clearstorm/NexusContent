...
| `core.schema.sections-map` Page-level sections map projection | implemented | 0.2.10 | - | `getPage` projects the provider's ordered sections onto `page.sections` (a named map keyed by section type; repeated types are suffixed `_2`, `_3`, ... in order) plus `page.sectionsList` (the authoritative ordered list). Git pages author the map directly (`content/pages/<key>.json` → `sections: { type: data }`, JSON key order = section order); WordPress flexible/Gutenberg page content is normalized to the same ordered list and projected to the map. A page model never needs to declare its sections: the projection is not gated on component fields, and `strictSections` is removed. Collection items keep `data.sections` (the ordered `{ type, data }` list). |
...
| `provider.wordpress.settings` WordPress settings | implemented | 0.2.9 | - | `getSettings(key)` maps native `wp/v2/settings` into generic `SettingsContent`. With companion plugin `0.1.8`, `auto`/`companion` strategies prefer public contract-v1 `/settings` values authored through an ACF Pro/SCF option page and fall back to native settings when that route is unavailable. |
...
| `provider.wordpress.phase1.companion` WordPress Phase 1 companion wire contracts | implemented | 0.2.1 | - | Contract v1 envelopes use `{ contractVersion: 1, data, diagnostics? }`; namespace `nexuscontent/v1` exposes pages, posts, schema, capabilities, and (since plugin `0.1.8`) settings contracts. |
...
| `provider.wordpress.companion-plugin` WordPress companion plugin | implemented | 0.2.1 | - | Plugin `0.1.8` at `integrations/wordpress/nexuscontent`; local unit, static-analysis, contract, asset, packaging, and plugin-free/ACF Free/SCF integration tests pass. Editor-mode sections and SEO authoring cover pages/posts, and public contract-v1 routes include pages, posts, schema, capabilities, and site settings. |
...
| `provider.wordpress.companion-item-sections` Companion item sections | implemented | 0.2.3 | - | Companion collection items surface normalized sections as `data.sections` (`{ type, data }`), the same canonical shape Git blog posts author; section-less posts keep the raw-HTML fallback. The built-in `posts` collection routes to the companion `posts` routes (`companionRoute: "pages" \| "posts"`; custom collections without a companion route fall back or throw strictly); section media is normalized recursively (`image.url` → `MediaAsset src`); `data` also surfaces `excerpt` and `featuredImage`. Unit tests cover populated/empty sections, route selection, media conversion, fallback, and strict errors. |
...
| `provider.wordpress.preview` WordPress draft preview | implemented | 0.2.6 | - | The companion plugin mints short-lived, post-scoped preview tokens (`POST /nexuscontent/v1/preview-token`, requires `edit_posts`, transient-backed with a filterable TTL) and serves draft/scheduled content through the public `GET /nexuscontent/v1/preview/{token}/{id}` route, where the token is the auth. Invalid, expired, or mismatched tokens return 401 and are revoked on use. A Gutenberg "Open frontend preview" button opens the configured `preview_frontend_url` with `?token=...&id=...`. The astro-wordpress reference consumer adds a static `preview.astro` route that fetches the tokenized route and renders through the shared section components. |
| `provider.wordpress.webhook` WordPress outbound webhooks | implemented | 0.2.7 | - | The companion plugin dispatches opt-in, outbound-only change notifications on page/post create, update, trash, and restore. A `webhook_url` (absolute http(s)) is stored in `nexuscontent_settings`; an optional shared secret sits in a separate `nexuscontent_webhook_secret` option that is never echoed or logged. Each request POSTs a compact JSON payload (event, id, type, slug, status, title, modifiedAt, source) with an `X-NexusContent-Signature: sha256=<HMAC-SHA256 of the raw body>` header. Dispatch is best-effort and non-blocking, and never triggers rebuilds or other site mutations; the consuming frontend verifies the signature and decides what to do. The provider `capabilities()` reports `previewSupport` and `webhookSupport`. |
...
| `seo.resolution` Deterministic SEO resolution | implemented | 0.1.4 | - | Pure `resolveSeo` applies documented content and site-default fallback chains without mutating input; `NexusContent.resolvePageSeo` (added `0.2.10`) derives the defaults from the settings model's `siteName`/`defaultImage` convention. |
...
| `seo.astro-rendering` Astro SEO rendering | implemented | 0.1.4 | - | Consumer-owned reference components render metadata across the Astro examples; Core's `serializeJsonLd` (added `0.2.10`) escapes script-breaking characters for inline JSON-LD, vendor-specific page defaults come from `nexus.resolvePageSeo`, and only the deployment-owned canonical base URL stays per project. |
| `seo.automation` Advanced SEO automation | deferred | - | TBD | Automatic canonical URL construction (Core holding or inferring the deployment base), sitemaps, robots.txt, metadata scraping, keyword analysis, redirects, and provider-specific SEO plugin behavior require separate scope. The framework-neutral `makeCanonicalUrl` join primitive (added `0.2.10`) is supplied but requires an explicit consumer base URL. |
