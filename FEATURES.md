...
| `core.schema.sections-map` Page-level sections map projection | implemented | 0.2.9 | - | `getPage` projects the provider's ordered sections onto `page.sections` (a named map keyed by section type; repeated types are suffixed `_2`, `_3`, ... in order) plus `page.sectionsList` (the authoritative ordered list). Git pages author the map directly (`content/pages/<key>.json` → `sections: { type: data }`, JSON key order = section order); WordPress flexible/Gutenberg page content is normalized to the same ordered list and projected to the map. A page model never needs to declare its sections: the projection is not gated on component fields, and `strictSections` is removed. Collection items keep `data.sections` (the ordered `{ type, data }` list). |
