...
NexusContent Companion exposes normalized pages, posts, site settings, SEO, schema, capabilities, media, and diagnostics through WordPress REST routes. Content is read-only; the only write route is the admin-only project-contract push. Plugin 0.2.1 uses companion contract 1, supports signed, outbound-only change webhooks, and enables classic menu management even on block themes.
...
Run npm install, npm run build, npm run lint-js, npm run format:check, and npm run package. Packaging creates repository-root dist/nexuscontent-0.2.1.zip and excludes maps, source assets, tests, dependencies, local config, secrets, and dev configs.
