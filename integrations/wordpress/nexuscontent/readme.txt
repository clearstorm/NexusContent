...
NexusContent Companion exposes normalized pages, posts, site settings, SEO, schema, capabilities, media, and diagnostics through WordPress REST routes. Content is read-only; the only write route is the admin-only project-contract push. Plugin 0.2.2 uses companion contract 1, supports signed, outbound-only change webhooks, and enables classic menu management even on block themes.
...
Run npm install, npm run build, npm run lint-js, npm run format:check, and npm run package. Packaging creates repository-root dist/nexuscontent-0.2.2.zip and excludes maps, source assets, tests, dependencies, local config, secrets, and dev configs.
...
* The Section types toggle switches on the Settings > Integrations screen no longer collapse into a bare thumb circle floating over the label; the toggle layout now outguns the WordPress `form-table fieldset label` rule that previously broke it.
* The Frontend preview URL now accepts absolute http(s) URLs on local or non-standard-port hosts (for example `http://localhost:4321`), so the setting saves during local frontend development. Webhook URL validation remains deliberately strict because it targets a real outbound request.

= 0.2.1 =

* Registers `add_theme_support( 'menus' )` on `after_setup_theme`, so the classic Appearance > Menus screen is available even on block themes, which do not declare menu support by default. This exposes the `wp/v2/menus` and `wp/v2/menu-items` endpoints consumed by NexusContent navigation retrieval.
...
* The ACF Flexible FAQ `answer` field is now a plain Text Area instead of a WYSIWYG editor, matching the Gutenberg FAQ block and Git-authored content. WYSIWYG output previously shipped HTML (`<p>`/`<div>` wrappers) that surfaced as stray tags in consumers, which render `answer` as escaped text. The server-side fallback block renderer now `esc_html`es the answer instead of running it through `wpautop`, so plain-text answers stay plain. Existing FAQ items authored with rich HTML must be re-saved to clear any legacy HTML.

= 0.1.6 =

* ACF flexible sections now reach consumers with canonical field names: the normalizer reads the formatted ACF value so sub-fields are keyed by their names (`heading`, `body`, ...) instead of their raw ACF field keys (`field_nc_*`), fixing validation mismatches against consumer schemas.
* The Features section's `points` are normalized to flat strings: ACF authors them as a repeater of a `text` sub-field (`[{ text }]`), and the server now flattens them to `points: ["...", ...]`, matching the canonical contract and consumer schemas. Already-flat values are left unchanged.
